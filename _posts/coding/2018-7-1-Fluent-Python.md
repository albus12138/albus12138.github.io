...
- 变量是盒子的标签而不是盒子，类似于 C 中的指针，变量指向的是一个空间

- 变量之间的比较
	- 赋值 `==` ：赋值的过程就是先创建一个对象，然后将变量标签贴到这个盒子上，对一个已经赋值的变量重新赋值，就是将标签贴到另一个盒子上，如果将一个变量赋值给另一个变量，也可以认为是创建了一个别名(aliases)
	- `is`：要判断两个变量是不是指向同一个盒子，就需要比较两个变量的id，可以使用 `is` 关键字，值得注意的是 `is` 的执行速度快于 `==`

- 元组是相对不可变的，元组内的变量是不可变的，但是如果元组内部存在可变变量，这个可变变量内部的内容是可变的

- `copy` 默认是浅复制的

- 深复制对于一些人造的对象可能会出现问题，如将一些应当共享的内容独立复制出来，此外，对于一些存在循环引用的情况可能会引起一些复制过深的情况

- Python 传入函数的参数实际上是原变量的一个浅复制，所以当使用可变变量作为函数默认参数时会引起多个对象公用一个列表的情况，可以使用下面这段代码安全初始化
...
- 垃圾回收机制：Python 的gc机制是基于引用数的，当一个空间的引用数达到0时，就会被销毁，其他 Python 解释器实现了一些更复杂的gc机制。关于 `del`，并不是删除掉一个空间，而是删除掉一个引用。

- 弱引用：弱引用可以保留一个到目标的引用而不会增加目标空间的引用数，所以经常会在缓存中用到，不是所有的对象都可以使用弱引用
...
- 不可变对象的复制：不可变变量复制时不会生成一个新的空间，仅仅是产生一个新的引用

### Chapter 9 | Pythonic 对象

- 对象的表达：在每个面向对象的语言中至少有一个用字符串表示对象的方法，在 Python 中有两个 `repr()` 和 `str()`，通过 `__repr__` 和 `__str__` 实现

- classmethod：通过装饰器 `@classmethod` 可以声明函数的接受的第一个参数为其所属的类，通常被用来作为一个类的构造函数，如接收不同内容执行不同的初始化操作
...
- staticmethod：声明一个函数为静态函数，即不接受 self 也不接受 cls，用途不明，完全可以通过定义在类外面实现相同功能

- 格式化：通过 `__format__` 实现，注意在其中尽量不要硬编码类名，而是使用 `self.__name__`，便于继承

- Hashable：通过 `__hash__` 实现

- 私有、公有、保护：Python 的默认方法均为公开的， 对于私有和保护可以在变量命名前加 `__` (`_`对编译器无意义，但实际效果相同) 来告诉编译器这是一个私有方法，如你在类中定义一个私有变量 `__x` 那么他会变成 `demo._DemoClass__x`，但 Python 仅能防止无意中的数据访问，对于故意越界访问私有数据没有提供解决方案

- 节约空间：类的属性默认是通过字典保存的，这会造成极大地空间占用，通过将类的属性添加到 `__slots__` 中可以告知编译器将这些属性保存到元组中，以此节省字典元数据的开支，但这样就不支持动态添加属性了，你需要将全部属性预先添加进去，或将 `__dict__` 添加进去，但这样会影响节省空间的效果，此外，在继承时也需要重新声明，因此，通常在百万级别数据量以下因为操作繁琐没有必要

- 覆盖类属性：可以通过直接给类而不是实例化对象的属性赋值来覆盖类原有的属性

<strong>To be Continued... Last updated: Feb 2, 2019</strong>